  ...
  useEffect(() => {
    const loadProperties = async () => {
      try {
        ...
        console.log("Admin Properties Loaded:", data); // Debugging
  ...
  const handleStatus = async (id, status) => {
    ...
    if (status === "rejected") {
      const { value } = await Swal.fire({
        ...
        input: "textarea",
        inputPlaceholder: "Why rejecting this property?",
          ...
          <table className="w-full">
            ...
            <tbody>
              {properties.length > 0 ? properties.map((property) => (
                ...
                <tr>
                  <td colSpan="6" className="p-20 text-center text-slate-400 text-lg">
                    No properties found. Add some from owner dashboard.
                  </td>
                </tr>
              )}
            </tbody>
