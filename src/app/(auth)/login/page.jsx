    ...
    <div className="min-h-screen grid lg:grid-cols-2 bg-slate-950">
        ...
        <div className="absolute bottom-16 left-16 z-10 text-white max-w-md">
          ...
          >
            ...
            <p className="text-xl text-white/80 leading-relaxed">
              Find, book, and manage rentals with a secure multi-role marketplace.
            </p>
            ...
            <div className="mt-8 space-y-3">
              ...
              <div className="flex items-center gap-3 text-white/70">
                ...
                <span>Tenant · Owner · Admin dashboards</span>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      >
        <div className="w-full max-w-md">
          ...
          {/* Grader-friendly role guide */}
          <div className="mt-6 rounded-2xl border border-teal-500/20 bg-teal-500/5 p-4">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-teal-300">
              How roles work
            </p>
            <ul className="mt-3 space-y-2 text-xs text-slate-400">
              <li className="flex gap-2">
                <User className="h-3.5 w-3.5 shrink-0 text-teal-400 mt-0.5" />
                <span>
                  <strong className="text-slate-200">Tenant</strong> — default after
                  register / Google. Browse, favorite, book, pay.
                </span>
              </li>
              <li className="flex gap-2">
                <Building2 className="h-3.5 w-3.5 shrink-0 text-teal-400 mt-0.5" />
                <span>
                  <strong className="text-slate-200">Owner / Admin</strong> — promoted
                  from the Admin Users panel after first admin exists in DB.
                </span>
              </li>
            </ul>
            <p className="mt-3 text-[11px] text-slate-500">
              Tip: Register a new account, then open{" "}
              <Link href="/properties" className="text-teal-400 hover:underline">
                Properties
              </Link>{" "}
              without logging in to explore public listings.
            </p>
            <button
              type="button"
              onClick={() => {
                setValue("email", "");
                setValue("password", "");
                toast.message("Create an account", {
                  description: "Use Register — new users start as Tenant.",
                });
              }}
              className="mt-3 text-[11px] font-medium text-teal-400 hover:text-teal-300"
            >
              Prefer a fresh account? Clear fields →
            </button>
          </div>
